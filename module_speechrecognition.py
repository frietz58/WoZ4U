...
IDLE_RELEASE_TIME = 30     # seconds, for auto-detect mode: idle time (RMS below threshold) after which we stop recording and recognize
HOLD_TIME = 5             # seconds, minimum recording time after we started recording (autodetection)
...
class SpeechRecognitionModule(naoqi.ALModule):
    ...
    def __init__( self, strModuleName, strNaoIp ):
        try:
            ...
            self.stream.start()
            # self.stream.channels = 1  # causes error?
        ...
        try:
            ...
            if(not self.isCalibrating):
                if(self.isRecording):
                    ...
                    print "aSoundData.shape", aSoundData.shape
                    self.realtime_buffer = np.concatenate(self.buffer, axis=1)[0]
                    self.realtime_buffer = self.realtime_buffer.astype(np.float32)
                    # self.stream.start()
                    if len(self.realtime_buffer > 1000):
                        self.stream.write(aSoundData.astype(np.float32).T)
                        pass
                        # sd.play(self.realtime_buffer, 48000)
                        # sd.wait()

                        # # only makes weird noise?
                        # def callback(outdata, frames, time, status):
                        #     if status:
                        #         print(status)
                        #     print outdata.shape
                        #     data = self.realtime_buffer[:len(outdata)]
                        #     print data.shape
                        #     data = np.expand_dims(data, 1)
                        #     print data.shape
                        #     # outdata[:] = self.realtime_buffer[:len(outdata)]
                        #     outdata[:] = data
    ...
    def recognize(self, data):
        ...
        if (WRITE_WAV_FILE):
            ...
            # # THIS WORKS! NOW WE JUST NEED TO DO IT EARLIER!
            # chunk = 1024  
...
def main():
    ...
    time.sleep(3)
    # # works, but only playes the three seconds recorded and does not keep on recording :(
    # sd.playrec(SpeechRecognition.realtime_buffer, channels=1)
    ...
    # # only makes static nose :(
    # SpeechRecognition.stream.start()
    # SpeechRecognition.stream.write(SpeechRecognition.realtime_buffer)


    # # same problem, makes weird buzzing noise...
    # def callback(outdata, frames, time, status):
    #     if status:
    #         print(status)
    #     print outdata.shape
    #     data = SpeechRecognition.realtime_buffer[:len(outdata)]
    #     print data.shape
    #     data = np.expand_dims(data, 1)
    #     print data.shape
    #     # outdata[:] = self.realtime_buffer[:len(outdata)]
